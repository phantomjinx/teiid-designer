 ...
 *
 */
public class EquinoxStorageLoginDialog extends FormDialog {

    private static final String PREFIX = I18nUtil.getPropertyPrefix(EquinoxStorageLoginDialog.class);

    private static final String HELP_ID = Activator.PLUGIN_ID + ".sec_storage_context"; //$NON-NLS-1$
     ...
     * @param location
     */
    public EquinoxStorageLoginDialog(boolean confirmPassword, boolean passwordChange, String location) {
    ...
    private String getString(String theKey) {
        return Activator.UTIL.getString(new StringBuffer().append(PREFIX).append(theKey).toString());
    }
    ...
    protected void createButtonsForButtonBar(Composite parent) {
        okButton = createButton(parent, IDialogConstants.OK_ID, getString("buttonLogin"), true); //$NON-NLS-1$
        okButton.setEnabled(false);
        createButton(parent, IDialogConstants.CANCEL_ID, getString("buttonExit"), false); //$NON-NLS-1$
    }
    ...
    protected void createFormContent(IManagedForm mform) {
        ...
        descriptionSection.setText(getString("descriptionTitle")); //$NON-NLS-1$
        ...
        descriptionText.setText(getString("secureDescription"), true, true); //$NON-NLS-1$
        ...
        String titleMsg;
        if (confirmPassword)
            titleMsg = getString("passwordChangeTitle"); //$NON-NLS-1$
        else if (passwordChange)
            titleMsg = getString("messageLoginChange"); //$NON-NLS-1$
        else
            titleMsg = getString("dialogTitle"); //$NON-NLS-1$
        ...
        Label passwordLabel = toolkit.createLabel(passwordComposite, getString("labelPassword"), SWT.LEFT); //$NON-NLS-1$
        ...
        if (confirmPassword) {
            Label passwordConfLabel = toolkit.createLabel(passwordComposite, getString("labelConfirm"), SWT.LEFT); //$NON-NLS-1$
        ...
        showPassword = toolkit.createButton(passwordComposite, getString("showPassword"), SWT.CHECK | SWT.RIGHT); //$NON-NLS-1$
        ...
        if (location != null) {
            ...
            locationGroup.setText(getString("locationGroup")); //$NON-NLS-1$
    ...
    protected boolean validatePassword(ScrolledForm mainForm) {
        ...
        if ((password1 == null) || (password1.length() == 0)) {
            statusLabel.setText(getString("messageEmptyPassword")); //$NON-NLS-1$
            return false;
        }
        if (confirm != null) {
            String password2 = confirm.getText();
            if (!password1.equals(password2)) {
                statusLabel.setText(getString("messageNoMatch")); //$NON-NLS-1$
                return false;
            }
        }
