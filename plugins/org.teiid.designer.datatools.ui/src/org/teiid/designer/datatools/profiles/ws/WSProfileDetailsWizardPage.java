...
public class WSProfileDetailsWizardPage extends ConnectionProfileDetailsPage implements DatatoolsUiConstants {
    ...
    private void addListeners() {
        urlText.addListener(SWT.Modify, new Listener() {
            ...
            public void handleEvent(Event event) {
                ...
                setProperty(IXmlProfileConstants.URL_PROP_ID, urlStr);
            }
        });
        ...
        credentialsComposite.addSecurityOptionListener(SWT.Modify, new Listener() {
            @Override
            public void handleEvent(Event event) {
                setProperty(IXmlProfileConstants.SECURITY_TYPE_ID,
        ...
        credentialsComposite.addUserNameListener(SWT.Modify, new Listener() {
            @Override
            public void handleEvent(Event event) {
                setProperty(IXmlProfileConstants.USERNAME_PROP_ID,
        ...
        credentialsComposite.addPasswordListener(SWT.Modify, new Listener() {
            @Override
            public void handleEvent(Event event) {
                setProperty(IXmlProfileConstants.PASSWORD_PROP_ID,
    ...
    void updateState() {
        ...
        if (null != properties.get(IXmlProfileConstants.SECURITY_TYPE_ID) &&
        		!SecurityType.None.name().equals(properties.get(IXmlProfileConstants.SECURITY_TYPE_ID))) {
        	if (null == properties.get(IXmlProfileConstants.USERNAME_PROP_ID)
                    || properties.get(IXmlProfileConstants.USERNAME_PROP_ID).toString().isEmpty()) {
                    setErrorMessage(UTIL.getString("Common.Username.Error.Message")); //$NON-NLS-1$
                    return;
                }
                setErrorMessage(null);
                if (null == properties.get(IXmlProfileConstants.PASSWORD_PROP_ID)
                    || properties.get(IXmlProfileConstants.PASSWORD_PROP_ID).toString().isEmpty()) {
	...
	private boolean internalComplete(boolean complete) {
		...
		if (complete
				&& (null == properties.get(IXmlProfileConstants.URL_PROP_ID) || properties
						.get(IXmlProfileConstants.URL_PROP_ID).toString()
						.isEmpty())) {
			complete = false;
		}
		if (complete
				&& null != properties.get(IXmlProfileConstants.SECURITY_TYPE_ID) && (!SecurityType.None.name().equals(
						properties.get(IXmlProfileConstants.SECURITY_TYPE_ID)
								.toString()))) {
			if (complete
					&& (null == properties
							.get(IXmlProfileConstants.USERNAME_PROP_ID) || properties
							.get(IXmlProfileConstants.USERNAME_PROP_ID)
							.toString().isEmpty())) {
				complete = false;
			}
			if (complete
					&& (null == properties
							.get(IXmlProfileConstants.PASSWORD_PROP_ID) || properties
							.get(IXmlProfileConstants.PASSWORD_PROP_ID)
							.toString().isEmpty())) {
				complete = false;
			}
